...
Duplicated from benchmarks/rag_recall (same convention as tools.py — each
benchmark folder is self-contained) rather than imported, with the
docstring's example updated for this benchmark's shape: real embedding
calls cost money and hit the network; without a cache, re-running this
benchmark while iterating on phrasings would re-embed every answer from
scratch each time. Caches by ``sha256(model_id + text)`` so a change of
